...
export default function DeleteProduct() {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const res = await axios.delete(`127.0.0.1:5000/characters/${characterId}`);
                    ...
                    <Col md="6">
                        <FloatingLabel controlId="formId" label="ID" className="mb-2 mt-2">
                                <Form.Select aria-label="Select Character ID from the list" >
                                    ...
                                    { idNameList.map((char) => (
                                        <option key={char.id} value={char.id} onClick={handleChange}>id: {char.id}, name: {char.name}</option>
                                    ))}
                                </Form.Select>
